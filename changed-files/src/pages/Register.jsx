...
export function Register() {
  const { register, verifyEmail, updateUser } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setPending({ ...result, profile: payload });
    } catch (err) {
      setErrors((prev) => ({
        ...prev,
  ...
  if (pending && !showOnboarding) {
    return <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex items-center justify-center p-4 sm:p-6">
      <div className="max-w-md w-full bg-white rounded-3xl p-6 sm:p-8 shadow-2xl">
        <EmailVerification email={pending.email} onBack={() => setPending(null)}
          onResend={() => register(pending.profile)}
          onVerify={async (details) => {
            const verified = await verifyEmail(details);
            setRegisteredUser(verified);
