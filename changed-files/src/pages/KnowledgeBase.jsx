...
export function KnowledgeBase() {
  ...
  const handleOpenDetail = (item) => setActiveItem(item);
  const handleCommentsChange = useCallback((entryId, comments) => {
    setItems((previous) => previous.map((item) => item.id === entryId ? { ...item, comments } : item));
    setActiveItem((previous) => previous?.id === entryId ? { ...previous, comments } : previous);
  }, []);
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-150">
          <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl border border-slate-100 overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-xs sm:text-sm text-slate-800">
              ...
              <CommentThread key={activeItem.id} entryId={activeItem.id} authUserId={user?.authId}
