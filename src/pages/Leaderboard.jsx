...
export function Leaderboard() {
  ...
  // Privileges Section View Mode: 'carousel' | 'grid'
  const [privilegeViewMode, setPrivilegeViewMode] = useState('carousel');

  // 3D DepthCarousel items representing the privileges
  const carouselItems = useMemo(() => {
    const list = activeBoard === 'students' ? STUDENT_PERK_TIERS : ALUMNI_PERK_TIERS;

    const items = list.map((perk, idx) => ({
      id: `perk-${idx}`,
      content: (
        <div className="h-full flex flex-col justify-between space-y-3 text-left select-none p-1">
          <div className="space-y-3">
            {/* Header */}
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-2.5">
                <span className="text-2xl w-10 h-10 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-center shrink-0 shadow-2xs">
                  {perk.icon}
                </span>
                <div>
              ...
              <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider shrink-0 ${perk.badgeBg}`}>
                {perk.rank}
              </span>
            </div>
            ...
            <div className="space-y-2.5 text-xs leading-relaxed">
              {perk.problem && (
                <div className="p-3 rounded-2xl bg-rose-50 border border-rose-200/80 text-rose-900 space-y-1">
              ...
              {perk.solution && (
                <div className="p-3 rounded-2xl bg-indigo-50/70 border border-indigo-200/70 text-indigo-950 space-y-1">
              ...
              {perk.trustFactor && (
                <div className="p-2.5 rounded-xl bg-emerald-50 border border-emerald-200/70 text-emerald-950 flex items-start gap-2">
              ...
              {perk.reality && (
                <div className="p-3 rounded-2xl bg-emerald-50/80 border border-emerald-200 text-emerald-950 space-y-1">
              ...
              {perk.result && (
                <div className="p-2.5 rounded-xl bg-blue-50 border border-blue-200/70 text-blue-950 flex items-start gap-2">
              ...
              {perk.campusPrivileges && (
                <div className="p-3 rounded-2xl bg-purple-50/80 border border-purple-200 text-purple-950 space-y-1">
              ...
              {perk.citation && (
                <div className="p-3 rounded-2xl bg-amber-50/80 border border-amber-200 text-amber-950 space-y-1">
    ...
    // Add 3rd card: Symbiotic Flywheel Preview Card
    items.push({
      id: 'perk-flywheel-card',
      content: (
        <div className="h-full flex flex-col justify-between space-y-3 text-left select-none bg-gradient-to-br from-slate-900 to-indigo-950 text-white -m-4 sm:-m-5 p-5 sm:p-6 rounded-[22px]">
          <div className="space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-2.5">
                <span className="text-2xl w-10 h-10 rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center shrink-0">
                  🔄
                </span>
                <div>
                  <h4 className="text-sm sm:text-base font-black text-white leading-snug">
                    The Symbiotic "Knowledge Flywheel"
                  </h4>
                  <p className="text-[11px] font-semibold text-indigo-300 mt-0.5">
                    {activeBoard === 'students'
                      ? 'Turn Lab Guides Into Corporate Referrals'
                      : 'First-Look Junior Talent & Referral Bonuses'}
                  </p>
                </div>
              </div>
              <span className="text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider shrink-0 bg-indigo-500 text-white">
                Closed-Loop Engine
              </span>
            </div>

            <div className="space-y-2 text-xs">
              <div className="p-2 rounded-xl bg-white/[0.06] border border-white/10 flex items-center gap-2">
                <span className="w-5 h-5 rounded-md bg-indigo-500/30 text-indigo-300 flex items-center justify-center font-bold text-[10px]">1</span>
                <span className="text-slate-200"><strong>Senior SOP Contribution:</strong> Industry playbooks & architectures.</span>
              </div>
              <div className="p-2 rounded-xl bg-white/[0.06] border border-white/10 flex items-center gap-2">
                <span className="w-5 h-5 rounded-md bg-blue-500/30 text-blue-300 flex items-center justify-center font-bold text-[10px]">2</span>
                <span className="text-slate-200"><strong>Junior Learning:</strong> Solving lab bugs, farming merit points.</span>
              </div>
              <div className="p-2 rounded-xl bg-white/[0.06] border border-white/10 flex items-center gap-2">
                <span className="w-5 h-5 rounded-md bg-purple-500/30 text-purple-300 flex items-center justify-center font-bold text-[10px]">3</span>
                <span className="text-slate-200"><strong>Referral Token:</strong> Direct guaranteed connection without LinkedIn spam.</span>
              </div>
              <div className="p-2 rounded-xl bg-white/[0.06] border border-white/10 flex items-center gap-2">
                <span className="w-5 h-5 rounded-md bg-emerald-500/30 text-emerald-300 flex items-center justify-center font-bold text-[10px]">4</span>
                <span className="text-slate-200"><strong>Referral Payout:</strong> Senior collects ₹30k–₹1.5L bonus on hire.</span>
              </div>
            </div>
          </div>
          ...
          <div className="pt-2 border-t border-white/10 flex items-center justify-between text-[11px] text-indigo-300 font-semibold">
            ...
            <span className="text-emerald-400 font-bold flex items-center gap-1">
              100% Verified <Repeat className="w-3.5 h-3.5 text-emerald-400" />
            </span>
          </div>
        </div>
      ),
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* View Mode Switcher: 3D DepthCarousel vs Grid */}
            <div className="bg-slate-100 p-1 rounded-xl flex items-center gap-1 border border-slate-200 text-xs font-bold shrink-0">
              <button
                type="button"
                onClick={() => setPrivilegeViewMode('carousel')}
                className={`px-3 py-1.5 rounded-lg transition flex items-center gap-1.5 ${
                  privilegeViewMode === 'carousel'
                    ? 'bg-white text-indigo-700 shadow-xs'
                    : 'text-slate-500 hover:text-slate-800'
                }`}
                title="Interactive 3D Depth Carousel"
              >
                <span>🎴 3D Carousel</span>
              </button>
              <button
                type="button"
                onClick={() => setPrivilegeViewMode('grid')}
                className={`px-3 py-1.5 rounded-lg transition flex items-center gap-1.5 ${
                  privilegeViewMode === 'grid'
                    ? 'bg-white text-indigo-700 shadow-xs'
                    : 'text-slate-500 hover:text-slate-800'
                }`}
        ...
        {/* 3D DepthCarousel View */}
        {privilegeViewMode === 'carousel' ? (
          <div className="space-y-2">
            <div className="relative w-full h-[490px] sm:h-[510px] overflow-hidden rounded-3xl bg-slate-50/60 border border-slate-200/80 p-2 sm:p-4 flex items-center justify-center">
              <DepthCarousel
                items={carouselItems}
                depth={180}
                spread={80}
            ...
            <p className="text-center text-[11px] font-semibold text-slate-400">
              💡 <em>Drag sideways, use arrow keys, or click any card in the stack to rotate in 3D</em>
            </p>
          </div>
        ) : (
